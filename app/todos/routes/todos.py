...
@router.get("/{user_id}")
async def read_uncomplete(db:db_dependency, user:user_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail='Authentication Failed')
    return db.query(Todo).filter(Todo.owner_id == user.get("id")).filter(Todo.complete == False).all()

@router.get("/{user_id}")
async def read_complete(db:db_dependency, user:user_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail='Authentication Failed')
    return db.query(Todo).filter(Todo.owner_id == user.get("id")).filter(Todo.complete == True).all()
